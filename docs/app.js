...
function initSignup() {
  ...
  const setMsg = (text, cls) => {
    ...
    msg.className = "signup-msg" + (cls ? " " + cls : "");
  };
  form.addEventListener("submit", async (e) => {
    ...
    if (EMAIL_WORKER.includes("REPLACE")) {
      setMsg("Email signup isn't wired up yet — deploy the email worker first.", "err");
      return;
    }
    btn.disabled = true;
    setMsg("Subscribing…", "");
    try {
      const res = await fetch(`${EMAIL_WORKER}/subscribe`, {
        method: "POST",
